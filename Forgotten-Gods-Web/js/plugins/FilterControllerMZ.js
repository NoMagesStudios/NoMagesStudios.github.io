 ...
 * @text Displacement???
 * @desc ?????? 2 ????????img/pictures/ ?????????(512x512, 1024x1024??)
 * @type file
 * @dir img/pictures/
 ...
 * ????        :Chars
 * ?????(id)    :SepcificChar
 * ???????(id)  :ExcludeSepcificChar
 * ?????       :Pictures
 * ??????(id)   :SpecificPicture
 * ????????(id) :ExcludeSpecificPicture
 * 
 * 
 * ???????????
 * ------------------
 ...
 *  ????????????????????????
 * 
 *   ?: <Filter:???,godray,TileAndCharsAndParallax>
 *       <Filter:???,twist,SpecificChar,-1>
 * 
 * 
 *   <SetFilter:???,[?????????????...]>
 *    - setFilter????
 *      ??<Filter>???????????????????
 * 
 *   ?: <Filter:???,godray,FullScreen>
 *      <SetFilter:???,-30,0.5,2.5,1>
 * 
 * 
 *   <SetFilterSpeed:???,??????>
 *    - setFilterSpeed????
...
(function() {
	...
	ConfigManager.applyData = function(config) {
		...
		if (config['TKMFilterEnabledAll'] === undefined) { // ???????
